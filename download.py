...
def extract_pdf_link(
    ...
    nameElement = soup.find('div', attrs={'data-test': 'book-title', 'class': 'page-title'})
    text = nameElement.find('h1').text.replace(" ", "_")
    # file_name = url[url.rfind("=")+1:]
    wget.download(pdf_link, "{}/{}.pdf".format(saved_folder, text))
    ...
    with codecs.open(springer_link_file, "r", encoding="utf-8") as f:
        ...
        for i, link in enumerate(lines):
            try:
                ...
                pdf_link, name = extract_pdf_link(link, saved_folder)
                #name = extract_pdf_name(link)
                print(name)
